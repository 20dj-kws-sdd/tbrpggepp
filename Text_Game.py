...
def load_world(var_name, vals):
    ...
    "enemy_room":EnemyRoom, "item_room":LootRoom, "victory_room":WinningRoom,
...
"""
        ...
        intro_text = "A Text-Based Adventure Game, programming by James Denovan\n(c) 2020 - See GPLv3 license for details\n\n\
==============================================\nPlease select one of the following questlines by name:\n"
        worlds_list = []
        world_path = os.path.normcase(os.path.dirname(os.path.realpath(__file__))) + '/worlds/'
        if len(sys.argv) != 1:
        ...
        for file in os.scandir(world_path):
            ...
            if filename[0] == "formats" and filename[1] == "json":
                continue  # ignore formats file
            if filename[1] == "json":
                worlds_list.append(filename[0])
